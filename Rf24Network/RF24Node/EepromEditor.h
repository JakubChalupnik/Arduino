...
void DisplayEeprom (void) {
  ...
  Vt100GotoXY (3, 2);
  memcpy (Buffer, Eeprom.Id, NODE_ID_SIZE);
  Buffer[NODE_ID_SIZE] = 0;
  printf_P (PSTR("%3.3o: %s"), Eeprom.NodeAddress, Buffer);
    ...
    switch (c) {
      ...
      case 'A':
        ...
        if (GetString (Buffer, 3)) { // Successfull entry finished by <Enter>
          Buffer [3] = 0;
          sscanf (Buffer, "%o", &Word);
          Eeprom.NodeAddress = Word;
        }
        Vt100ShowCursow (false);
        break;
      ...
      case 'I':
        ...
        if (GetString (Buffer, NODE_ID_SIZE)) { // Successfull entry finished by <Enter>
          ...
          memcpy (Eeprom.Id, Buffer, NODE_ID_SIZE);
        }
        Vt100ShowCursow (false);
        break;
